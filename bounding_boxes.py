...
def box_in_box(b, outer_box) :
    ...
    return (x1-5) < b[0] < (x2) and (x1) < b[2] < (x2+5) and (y1-5) < b[1] < (y2) and (y1) < b[3]< (y2+5)     
...
def non_max_suppression_fast(boxes, overlapThresh = 0.99):
...
def get_bboxes(orig_img, gray_img):
    ...
    mser_boxes = msers[1]
    mser_boxes = [[b[0], b[1], b[0] + b[2] - 1, b[1] + b[3] - 1] for b in mser_boxes]
    mser_boxes = non_max_suppression_fast(mser_boxes)

    ordered_words = word_seq(rescale(bboxes, rW, rH))
    letters = [word_seq(l) for l in link_letters_to_words(mser_boxes, ordered_words)]

    for (startX, startY, endX, endY) in ordered_words:
        cv2.rectangle(orig_img, (startX, startY), (endX, endY), (0, 255, 0), 2)
#    for _ in letters:
 #       for b in _ :
  #          cv2.rectangle(orig_img, (b[0], b[1]), (b[2], b[3]), (255,0,0), 1)
    
    cv2.imshow("Text Detection", orig_img)
    cv2.waitKey(0)
    
    letter_imgs = [[orig_img[b[1] : b[3] , b[0] : b[2] ] for b in _] for _ in letters] 
